...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("football", Key.RETURN);
  await driver.wait(until.titleIs("football - Google Search"), 1000);
  ...
  await driver.findElement(By.name("q")).sendKeys("hockey", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("hockey - Google Search"), 1000);
});
